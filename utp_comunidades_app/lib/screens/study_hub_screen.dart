...
import 'package:rive/rive.dart' hide LinearGradient, Image, RadialGradient;
...
class EstudIAColors {
  static const Color primary = Color(0xFFC8102E);    // Rojo UTP marca
...
class _StudyHubScreenState extends State<StudyHubScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Carga automática de cursos al entrar
    Future.microtask(() {
      if (mounted) {
              ...
              child: FadeTransition(
                ...
                child: Consumer<StudyProvider>(
                  builder: (context, studyProvider, _) {
                    if (studyProvider.error != null && studyProvider.courses.isEmpty) {
                    ...
                    if (studyProvider.courses.isEmpty && !studyProvider.isLoading && studyProvider.error == null) {
        ...
        child: isAI 
          ? MarkdownBody(
              ...
              styleSheet: MarkdownStyleSheet(
                p: TextStyle(color: Colors.black87, fontSize: 14, fontFamily: 'Montserrat', height: 1.5),
                strong: TextStyle(color: EstudIAColors.primary, fontSize: 14, fontWeight: FontWeight.w700, fontFamily: 'Montserrat'),
                listBullet: TextStyle(color: EstudIAColors.primary, fontSize: 14),
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Stack(
              ...
              children: [
                ShaderMask(
                  ...
                  child: Image.asset(
                    'assets/estudia_mascot.png',
                    width: 200,
                    height: 200,
                    fit: BoxFit.contain,
                  ),
                ).animate(onPlay: (c) => c.repeat(reverse: true))
                 .moveY(begin: -10, end: 10, duration: const Duration(seconds: 2), curve: Curves.easeInOut),
              ],
            ),
            ...
            const SizedBox(height: 16),
            Text(
              'Crea tu primer curso para que EstudIA pueda ayudarte a resumir, estudiar y aprobar con éxito.',
              style: TextStyle(
                fontSize: 16,
            ...
            _buildCreateFirstCourseButton(context),
          ],
        ),
      ),
    );
  ...
  Widget _buildCreateFirstCourseButton(BuildContext context) {
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        elevation: 8,
        shadowColor: EstudIAColors.primary.withOpacity(0.4),
      ),
      child: const Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          ...
          SizedBox(width: 8),
          Text('Crear mi primer curso', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ],
      ),
...
class _StudyCourseDetailScreenState extends State<StudyCourseDetailScreen>
  ...
  void _showAISummaryDialog({String? materialId}) {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) {
          // Fetch summary on first build
          if (isLoading && summaryText == null && errorMessage == null) {
            WidgetsBinding.instance.addPostFrameCallback((_) async {
              try {
                final provider = context.read<StudyProvider>();
                final aiResponse = await provider.summarizeMaterial(materialId!);
                if (aiResponse != null) {
                  setModalState(() {
                  ...
                  setModalState(() {
                    errorMessage = 'No se pudo generar el resumen. Intenta de nuevo.';
                    isLoading = false;
                  });
                }
              } catch (e) {
                          ...
                          Container(
                            ...
                            child: MarkdownBody(
                              ...
                              styleSheet: MarkdownStyleSheet(
                                ...
                                strong: const TextStyle(fontSize: 16, height: 1.5, color: Colors.black87, fontWeight: FontWeight.bold),
  ...
  void _showAIChatDialog(BuildContext context, {String? initialMessage}) {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) {
          if (!hasInitialized && initialMessage != null) {
            hasInitialized = true;
            WidgetsBinding.instance.addPostFrameCallback((_) async {
              setModalState(() {
                messages.add({
              ...
              try {
                final provider = context.read<StudyProvider>();
                final response = await provider.askQuestion(widget.course.id, initialMessage);
                ...
                setModalState(() {
                  messages.add({
                    'isAI': true,
                    'message': 'Lo siento, ocurrió un error: $e',
                  });
                  isLoading = false;
                });
              }
                      ...
                      GestureDetector(
                        onTap: isLoading
                            ...
                            : () async {
                                ...
                                try {
                                  final provider = context.read<StudyProvider>();
                                  final response = await provider.askQuestion(widget.course.id, text);
              ...
              GestureDetector(
                onTap: () {
                  if (textController.text.isNotEmpty) {
                    Navigator.pop(context);
                    final question = 'Explica el concepto: ${textController.text}';
                    _showAIChatDialog(context, initialMessage: question);
                  }
                },
                child: Container(
                  width: double.infinity,
  ...
  void _generateQuizWithOptions(int count, String difficulty) async {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => Dialog(
    ...
    try {
      final success = await context.read<StudyProvider>().generateQuiz(widget.course.id, count: count, difficulty: difficulty);
      if (mounted) {
        Navigator.pop(context); // Close loading dialog
        if (success) {
          final questions = context.read<StudyProvider>().questions;
          if (questions.isNotEmpty) {
            _startQuiz(questions);
          } else {
            showEstudIANotification(context, 'No se encontraron preguntas en el cuestionario.', isError: true);
          }
        } else {
          showEstudIANotification(context, 'No se pudo generar el cuestionario.', isError: true);
        }
      }
    } catch (e) {
      if (mounted) {
        Navigator.pop(context); // Close loading dialog
        showEstudIANotification(context, 'Error al generar el cuestionario: $e', isError: true);
      }
    }
  }
                ...
                child: SafeArea(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        'Revisa las preguntas y sus respuestas correctas explicadas',
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16),
                          ...options.entries.map((e) => Container(
                            margin: const EdgeInsets.only(bottom: 8),
                            padding: const EdgeInsets.all(12),
                            decoration: BoxDecoration(
                              color: e.key == q.correctOption ? Colors.green.shade50 : Colors.white,
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(
                                color: e.key == q.correctOption ? Colors.green.shade200 : Colors.grey.shade300,
                              ),
                            ),
                            child: Row(
                              children: [
                                Text(
                                  ...
                                  style: TextStyle(
                                    fontWeight: FontWeight.bold,
                                    color: e.key == q.correctOption ? Colors.green.shade700 : Colors.grey.shade700,
                                  ),
                                ),
                                Expanded(
                                  child: Text(
                                    e.value.toString(),
                                    style: TextStyle(
                                      color: e.key == q.correctOption ? Colors.green.shade700 : Colors.black87,
                                    ),
                                  ),
                                ),
                                if (e.key == q.correctOption)
