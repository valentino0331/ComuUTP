...
const initDatabase = async () => {
  try {
    ...
    // Migración 6: Agregar columna session_id a usuarios para sesión única
    try {
      await pool.query(`
        ALTER TABLE usuarios
        ADD COLUMN IF NOT EXISTS session_id TEXT;
      `);
      console.log('✅ Columna session_id en usuarios verificada');
    } catch (err) {
      console.log('ℹ️ Columna session_id:', err.message.substring(0, 100));
    }
    ...
    try {
      ...
      await pool.query(`
        CREATE TABLE IF NOT EXISTS study_materials (
          ...
          uploaded_by_user_id INTEGER REFERENCES usuarios(id) ON DELETE CASCADE,
          name VARCHAR(255) NOT NULL,
      ...
      try {
        await pool.query('ALTER TABLE study_materials RENAME COLUMN user_id TO uploaded_by_user_id');
      } catch (e) {}
    ...
    try {
      ...
      // Asegurar que la columna user_id exista si la tabla ya había sido creada antes
      await pool.query(`
    ...
    try {
      ...
      await pool.query(`
        ...
        CREATE INDEX IF NOT EXISTS idx_study_materials_user ON study_materials(uploaded_by_user_id);
