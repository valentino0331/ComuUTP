...
exports.uploadMaterial = async (req, res) => {
  try {
    ...
    if (req.file) {
      try {
        // Subir a Cloudinary - PÚBLICO para que pdf.service pueda acceder
        const result = await cloudinary.uploader.upload(req.file.path, {
          resource_type: 'raw',
          folder: 'estudia_materials',
          type: 'upload',  // ✅ PÚBLICO - permite acceso sin autenticación
...
exports.deleteCourse = async (req, res) => {
  try {
    const userId = req.user.id;
    const { courseId } = req.params;
    console.log('DEBUG: Delete course request:', { userId, courseId });
