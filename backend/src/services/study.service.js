...
class StudyService {
      ...
      try {
        ...
        await pool.query(
          `INSERT INTO ai_responses 
           ...
           RETURNING *`,
          [responseId, userId, courseId === 'general' ? null : courseId, 'answer', answerContent, question, false]
        );
      } catch (dbError) {
  ...
  async extractTextFromMaterial(material) {
    try {
      if (!material.file_url) return '';
      // A veces file_type puede no ser 'pdf', pero intentaremos si la URL tiene extensión .pdf
      if (material.file_type !== 'pdf' && !material.file_url.toLowerCase().endsWith('.pdf')) {
        return '';
      }
      
      console.log('📄 Extrayendo texto del PDF:', material.file_url);
      ...
      let text = data.text || '';
      // Limitar longitud para no exceder tokens (15000 chars ~ 3000-4000 tokens)
      if (text.length > 15000) {
        text = text.substring(0, 15000) + '\\n... [El resto del documento fue omitido]';
      }
      return text;
    } catch (err) {
  ...
  async generateAISummary(material) {
    ...
    if (GEMINI_KEY) {
      try {
        ...
        const materialContent = await this.extractTextFromMaterial(material);
...
${materialContent ? materialContent : '(No se pudo extraer el texto. Basa el resumen en el nombre del material)'}
...
Formato el resumen con emojis y markdown para que sea fácil de leer.`
    ...
    // Fallback inteligente si Gemini falla o no está configurado
    console.log('⚠️ Usando fallback para resumen');
    // Generar un pequeño resumen extractivo usando el inicio del texto extraído
    const text = await this.extractTextFromMaterial(material);
    if (text) {
      const excerpt = text.substring(0, 500).replace(/\n/g, ' ').trim();
      return `📄 **Resumen de "${material.name || 'documento'}"**\n\nAquí tienes un vistazo del contenido extraído:\n"${excerpt}..."\n\n💡 _Nota: La integración completa de IA avanzada está temporalmente inactiva o sin clave configurada, pero el documento ha sido procesado._`;
    }
    ...
    if (GEMINI_KEY) {
      try {
        ...
        for (const material of materials) {
          const text = await this.extractTextFromMaterial(material);
          if (text) {
            allContent += `\\n--- Contenido de ${material.name} ---\\n${text}\\n`;
          }
        }
        if (allContent.length > 20000) {
          allContent = allContent.substring(0, 20000) + '\\n... [Texto truncado]';
        }
...
${allContent ? allContent : '(No se pudo extraer el texto. Basa las preguntas en el nombre de los materiales)'}
...
SOLO devuelve el JSON, sin texto adicional.`;
    ...
    // Extraer texto para basar al menos la primera pregunta en el contenido real
    let sampleContent = '';
    if (materials.length > 0) {
       sampleContent = await this.extractTextFromMaterial(materials[0]);
    }
    
    return this.generateSmartQuestions(count, difficulty, materialNames, sampleContent);
  }
  ...
  async generateAIAnswer(courseId, question) {
    ...
    if (courseId && courseId !== 'general') {
      try {
        ...
        console.log('Error fetching context for chat:', err.message);
      }
    }
        ...
        const response = await fetch(`${GEMINI_API_URL}?key=${GEMINI_KEY}`, {
          ...
          body: JSON.stringify({
            contents: [{
              ...
              parts: [{
                text: `${ESTUDIA_PERSONALITY}
${contextText ? `\\nCONTEXTO DEL CURSO (Materiales de estudio):\\n${contextText}\\n` : ''}
PREGUNTA DEL ESTUDIANTE: "${question}"

Responde de forma natural, útil y conversacional. Utiliza el contexto del curso si la pregunta está relacionada a los materiales. Si es un tema académico, explícalo bien. Si es una pregunta simple, responde amigablemente.`
              }]
            }],
            generationConfig: {
              temperature: 0.8,
    ...
    try {
      ...
      const response = await fetch(HF_API_URL, {
        ...
        body: JSON.stringify({
          inputs: `<s>[INST] Eres EstudIA, un asistente académico amigable. ${contextText ? `\nAquí tienes la información del PDF/material:\n${contextText.substring(0, 5000)}\n\nBasado en esto, r` : 'R'}esponde de forma natural y conversacional: "${question}" [/INST]`,
          parameters: {
            max_new_tokens: 600,
            temperature: 0.8,
...
¿Qué información exacta necesitas sobre "${qClean}"? ✨`;
  ...
  async deleteCourse(courseId, userId) {
    try {
      console.log('DEBUG: Attempting to delete course:', { courseId, userId });
      
      // Primero buscar el curso sin filtrar por usuario para ver qué hay
      const allResults = await pool.query(
        'SELECT id, user_id FROM study_courses WHERE id = $1',
        [courseId]
      );
      console.log('DEBUG: Course search result:', allResults.rows);
      ...
      if (checkResult.rows.length === 0) {
        console.log('DEBUG: Ownership check failed:', { courseId, userId });
      ...
      await pool.query('DELETE FROM study_courses WHERE id = $1', [courseId]);
